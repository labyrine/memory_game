...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_summan_lataus_ei_muuta_kortin_saldoa(self):
        self.kortti.lataa_rahaa(-200)

        self.assertEqual(self.kortti.saldo_euroina(), 10.0)

    def test_kun_kortilla_rahaa_edullisen_lounaan_verran_voi_ostaa_edullisen_lounaan(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo_euroina(), 0.0)

    def test_kun_kortilla_rahaa_maukkaan_lounaan_verran_voi_ostaa_maukkaan_lounaan(self):
